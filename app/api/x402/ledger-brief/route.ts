...
export async function GET(req: NextRequest) {
  const address = req.nextUrl.searchParams.get("address") ?? "GBBD47IF6LWK7P7MDEVSCWR7DPUWV3NY3DTQEVFL4NAT4AQH3ZLLFLA5";
  ...
  const resourceUrl = `${req.nextUrl.origin}/api/x402/ledger-brief`;
  ...
  const requirements: PaymentRequirements = {
    ...
    amount: "50000", // 0.005 USDC (7 decimales)
    ...
    extra: {
      ...
      inputHash: Buffer.from(`${address}|${windowParam}`).toString("base64url"),
    },
  };
  ...
  const result = {
    ...
    summary: {
      accountExists: true,
      lastLedgerSequence: 4458902,
      activeTrustlines: 3,
      operationsCount24h: 142,
      riskLevel: "LOW",
    },
    events: [
      { type: "payment_received", asset: "USDC", amount: "50.00", timestamp: new Date(Date.now() - 3600000).toISOString() },
      { type: "contract_invocation", contract: "CA...REGISTRY", method: "stake_and_publish", timestamp: new Date(Date.now() - 7200000).toISOString() },
    ],
    anomalies: [],
  ...
  return NextResponse.json(
    {
      ok: true,
      serviceId: "stellar-ledger-brief",
      result,
      delivery: {
        status: "delivered",
        ...
        message: "Stellar ledger analytics delivered successfully via x402.",
      },
      payment: {
