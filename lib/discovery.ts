...
const aliases: Record<string, string[]> = {
  riesgo: ["risk", "security"],
  swap: ["swap", "liquidity", "defi"],
  contrato: ["contract", "soroban"],
  mercado: ["market", "market-data"],
  libro: ["ledger"],
  cuenta: ["ledger", "stellar"],
  herramienta: ["mcp"],
  datos: ["analytics", "market-data"],
  oraculo: ["oracle", "price-feed"],
  precio: ["price", "price-feed", "quote"],
};
...
export function rankServices(items: PaidService[], query: string): RankedService[] {
  ...
  if (!queryTokens.length)
    return items.map((service) => ({ service, score: 0, reasons: ["orden estable del catálogo"] }));
  const expanded = new Set(queryTokens.flatMap((token) => [token, ...(aliases[token] ?? [])]));
  return items
    .map((service) => {
      const name = new Set<string>(tokens(service.name));
      ...
      for (const term of expanded) {
        if (name.has(term)) {
          score += 5;
          reasons.push(`nombre coincide con “${term}”`);
        } else if (tags.has(term)) {
          score += 3;
          reasons.push(`tag coincide con “${term}”`);
        } else if (asset.has(term)) {
          score += 3;
          reasons.push(`activo coincide con “${term.toUpperCase()}”`);
        } else if (kind.has(term)) {
          score += 2;
          reasons.push(`tipo ${term.toUpperCase()}`);
        } else if (description.has(term)) {
          score += 1;
          reasons.push(`descripción menciona “${term}”`);
        }
      }
      return { service, score, reasons: [...new Set(reasons)] };
    })
...
export function validateServiceCard(card: ServiceCard): ValidationOutcome[] {
  ...
  rule(
    "resource.url",
    !!url && (url.protocol === "https:" || url.hostname === "localhost" || url.hostname === "127.0.0.1"),
    "URL HTTPS/local válida.",
  ...
  const inputs = new Set(card.input.map((i) => i.name));
  rule(
    "route.inputs",
    params.every((p) => inputs.has(p)),
    "Todos los parámetros tienen input declarado.",
    "Cada {parámetro} de la ruta debe existir en input.",
  );
  ...
  rule(
    ...
    "Precio positivo con precisión válida.",
    "amount debe ser decimal positivo (máximo 7 decimales).",
  );
  rule("payment.asset", card.payment.asset.trim().length > 0, "Asset declarado explícitamente.", "asset es obligatorio.");
  rule(
    "payment.destination",
    /^G[A-Z2-7]{55}$/.test(card.payment.destination),
    "Destino Stellar con formato público válido.",
    "destination debe ser una cuenta Stellar pública G… de 56 caracteres.",
  );
  rule(
    "provider.description",
    card.description.trim().length >= 20,
    "Descripción suficiente para discovery.",
