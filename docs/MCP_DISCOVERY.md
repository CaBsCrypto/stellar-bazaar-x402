...
Send JSON-RPC 2.0 `initialize`, then `tools/list` or `tools/call` to `/api/mcp` with `Accept: application/json, text/event-stream`. Stateless requests do not require a session ID. Registering a service persists it (Upstash Redis in production); later requests observe it through the read tools.