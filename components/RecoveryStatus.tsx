...
export function RecoveryStatus({events,record,available=false}:{events:ActivityEvent[];record?:RecoveryRecord;available?:boolean}) {
 ...
 return <section aria-label="Estado del pago y la entrega" className="recovery-status">
  <dl><div><dt>Pago</dt><dd>{state.payment}</dd></div><div><dt>Entrega</dt><dd>{state.delivery}</dd></div><div><dt>Última actualización recibida</dt><dd>{state.updated ? <time dateTime={state.updated}>{state.updated.replace("T", " ").slice(0, 19)}</time> : 'Sin actualizaciones recibidas'}</dd></div></dl>
