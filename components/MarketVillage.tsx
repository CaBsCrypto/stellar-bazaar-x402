...
export function MarketVillage(){
 ...
 return <section ref={root} className={"market-village "+(visible&&awake&&!open&&!reduced&&running&&ready?"is-visible":"")} aria-label="Mercado de posibilidades"><p className="market-eyebrow">Explora las posibilidades <span>· ejemplos ilustrativos</span></p><div className={"market-stage "+(ready?"has-three":"")} onFocusCapture={pause}><MarketScene selected={selected} elapsed={elapsed} ambient={story.ambient} active={visible&&awake&&!open} reduced={reduced} onReady={setReady}/><div className="market-fallback"><div className="market-platform" aria-hidden="true"/><div className="market-stalls">{marketStalls.map((item,index)=><button key={item.id} className={"market-booth "+(selected===index?"is-selected":"")} style={{"--stall-color":item.color,"--stall-dark":item.dark} as CSSProperties} aria-pressed={selected===index} aria-label={"Explorar "+item.name} onClick={()=>choose(index)}><Booth index={index}/><span className="market-booth-label">{item.name}</span></button>)}</div><span className="market-sign" aria-hidden="true">✦ BAZAAR</span></div><div className={"market-request "+(phase===0&&ready&&!reduced?"is-speaking":"")} aria-hidden={phase!==0||!ready||reduced}>{requests[selected]}</div></div><ol className="market-story-steps" aria-label="Cómo colabora tu agente">{["Tu solicitud","El agente explora","Recibes una muestra"].map((label,index)=><li key={label} aria-current={ready&&index===phase?"step":undefined}><span aria-hidden="true">{index+1}</span>{label}</li>)}</ol><div className="market-tour-controls"><span>{running&&!reduced&&ready?"Un paseo por las posibilidades":"Explora a tu ritmo"}</span><Button variant="quiet" disabled={reduced||!ready} onClick={()=>{if(running)pause();else resume();}}>{running&&!reduced&&ready?"Pausar recorrido":"Reanudar recorrido"}</Button></div>
 <div className="market-mobile-controls" onFocusCapture={pause}><Button variant="secondary" aria-label="Puesto anterior" onClick={()=>choose((selected+3)%4)}>←</Button><label>Especialidad<select value={selected} onChange={e=>choose(Number(e.target.value))}>{marketStalls.map((item,index)=><option key={item.id} value={index}>{item.name}</option>)}</select></label><Button variant="secondary" aria-label="Puesto siguiente" onClick={()=>choose((selected+1)%4)}>→</Button></div>
