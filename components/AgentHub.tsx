...
export function AgentHub({compact=false}:{compact?:boolean}){const [selected,setSelected]=useState(0);const refs=useRef<Array<HTMLButtonElement|null>>([]);
 ...
 <Panel role="tabpanel" id={`hub-panel-${selected}`} aria-labelledby={`hub-tab-${selected}`} tabIndex={0}>{selected<2?<AgentDirectConnectCard key={selected} compact={compact} role={selected===0?"buyer":"seller"}/>:<>{!compact&&<h2>Conecta tu cliente MCP</h2>}<p className="ui-muted">Usa esta configuración en un cliente compatible. No requiere pegar credenciales de la biblioteca. Revisa las herramientas disponibles en el playground.</p><CopyText text={config} label="Configuración MCP" compact={compact}/><div className="ui-actions"><ButtonLink href="/webmcp-playground" variant="secondary">Explorar herramientas WebMCP</ButtonLink><ButtonLink href="/docs" variant="quiet">Documentación de integración</ButtonLink></div></>}</Panel></>;
}
