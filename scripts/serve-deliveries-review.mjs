import { reviewDeliveries, reviewFilePaths } from "../lib/review-deliveries.ts";
...
const second = {
  ...
  manifest: {
    ...first.manifest,
    versionId: "script-v2",
    previousVersionId: first.manifest.versionId,
    versionLabel: "Versión 2 · prueba",
    summary: "Segunda versión simulada; el original se conserva.",
  },
};
...
const deliveryHandlers = createDeliverableHandlers({
  ...
  store: () => ({
    list: async () => records,
    usage: async () => 0,
    get: async (owner, id) => records.find((r) => r.manifest.versionId === id),
    reserve: async () => {
      throw Error();
    },
    confirm: async () => {
  ...
  objects: () => ({
    access: async (owner, version, file) => ({ url: reviewFilePaths[file.id] }),
    upload: async () => {
      throw Error();
    },
    verify: async () => {
...
http
    ...
    try {
      ...
      if (req.url.split("?")[0] === "/api/activity/connection") {
        ...
        response =
          ...
          new Response(null, { status: 405 });
      else if (req.url.split("?")[0] === "/api/deliveries")
        response = await deliveryHandlers.GET(request);
      else if (req.url.split("?")[0] === "/api/deliveries/access")
        response = await deliveryHandlers.access(request);
      else
  ...
  .listen(port, "127.0.0.1", () =>
    console.log(
      "Mock-only review at http://127.0.0.1:3213/history; test access: " +
        read +
        " / " +
        write,
    ),
