...
const publicKeys = [
  ...
  "WEBSITE_INTELLIGENCE_LIVE_APPROVED_CARD_HASH",
];
const redisKeys = [
...
function initials(run, mode) {
  return [
    {
      kind: "task-started",
      title: "Analizar example.com · informe real comprado",
    },
    { kind: "service-selected", title: "Website Intelligence seleccionado" },
    {
      ...
      title:
        mode === "mock"
          ...
          : "Solicitud Testnet · análisis real de HTML",
    },
  ].map((item, index) => ({
    ...item,
...
async function publicPreflight(run) {
  ...
  const requestBody = run?.requestBody ?? {
    ...
    operationId =
      run?.operationId ?? "website-live-paid-" + randomBytes(12).toString("hex");
  const recoveryToken =
  ...
  await requestWebsiteIntelligencePaymentChallenge({
    ...
    approvedPublicOrigin: providerOrigin,
    expectedPayTo: payTo,
    approvedCardHash: cardHash,
...
function acceptDeliveredBody(run, body) {
  ...
  assertPaidLiveReport(body.result, run.requestBody);
...
async function storeRun(run) {
  ...
  assertPaidLiveReport(run.result, run.requestBody);
...
async function execute(run) {
  ...
  assertPaidLiveReport(result.result, run.requestBody);
...
async function main() {
  ...
  if (["setup", "fixture", "serve"].includes(command)) throw Error("USE_EXISTING_PRIVATE_PILOT_SETUP_AND_SERVER");
  if (command === "status") {
